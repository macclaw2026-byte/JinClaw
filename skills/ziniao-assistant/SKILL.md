...
description: Control Ziniao Browser via the local Ziniao bridge. Use for store login, seller console navigation, finance/order/report exports, and other authenticated Ziniao-backed operations. On skill load or before first invoke, GET /zclaw/tools and treat returned name list as the only allowed tool strings; then POST /zclaw/tools/invoke. API key for invoke via ~/.zclaw/config.json or ZCLAW_API_KEY. On bridge failure stop the turn per skill.
---
# Ziniao Assistant
...
## Validated Seller-Console Playbooks

For seller-console work, keep this file lean and load the matching reference before acting:

- Generic seller-console navigation, login reuse, export verification, and failure patterns:
...
- **Use the already logged-in existing window/session only (workspace rule):** When the user already has a Ziniao store window open and logged in, do **not** open a new window or fresh session for the same task. Reuse the currently active logged-in store session/tab/target whenever possible, and prefer reading or interacting with the existing page state over relaunching the store. Only open/reopen the store if the user explicitly asks or if there is no usable existing logged-in session.
