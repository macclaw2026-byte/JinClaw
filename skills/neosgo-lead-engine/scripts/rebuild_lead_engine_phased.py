...
select
  *,
  ...
  concat_ws(' | ',
    concat('segment=', segment_primary), concat('buyer_type=', buyer_type), concat('role=', decision_role_level),
    concat('size=', size_band), concat('email=', cast(has_valid_email as varchar)),
    concat('phone=', cast(has_phone as varchar)), concat('website=', cast(has_website as varchar))
  ) as fit_reason
from scored
...
select
  segment_primary, buyer_type, decision_role_level, size_band, fit_tier, state,
  count(*) as lead_count,
...
''',
    'outreach_ready_leads': r'''
create or replace table outreach_ready_leads as
select
  md5(coalesce(email, website_host, company_key || '|' || state || '|' || city)) as queue_lead_id,
  file_id, company_name, contact_first, contact_last,
  coalesce(nullif(contact_full, ''), concat_ws(' ', contact_first, contact_last)) as contact_name,
  title, email, website, website_host, phone_digits as phone, city, state, industry,
  segment_primary, segment_secondary, buyer_type, decision_role_level, size_band, fit_score, fit_tier, fit_reason,
...
create table if not exists campaign_variants (
  campaign_id varchar, segment_primary varchar, fit_tier varchar, variant_code varchar, channel varchar,
  subject_template varchar, body_template varchar, cta_type varchar, active_flag boolean,
  created_at timestamp default current_timestamp
);
create table if not exists outreach_queue (
  queue_id varchar, queue_lead_id varchar, lead_file_id varchar, campaign_id varchar, variant_code varchar,
  channel varchar, priority_score double, scheduled_at timestamp, status varchar, attempt_no integer,
  created_at timestamp default current_timestamp
);
create table if not exists outreach_events (
  event_id varchar, queue_id varchar, lead_file_id varchar, event_type varchar, event_time timestamp, payload_json varchar
);
''',
    'campaign_variants_seed': r'''
...
def main():
    ...
    ap.add_argument('--start-from', default='normalized_contacts')
    args = ap.parse_args()
    ...
    state = {'phases': []}
    for phase in order[start_idx:]:
        con.execute(PHASES[phase])
        count = None
        if phase in ('normalized_contacts', 'deduped_contacts', 'scored_prospects', 'persona_summary', 'outreach_ready_leads') and table_exists(con, phase):
            count = table_count(con, phase)
        elif phase == 'outreach_queue_seed' and table_exists(con, 'outreach_queue'):
            count = table_count(con, 'outreach_queue')
        elif phase == 'campaign_variants_seed' and table_exists(con, 'campaign_variants'):
            count = table_count(con, 'campaign_variants')
        state['phases'].append({'phase': phase, 'count': count})
        Path(args.state_file).write_text(json.dumps(state, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
        print(json.dumps(state['phases'][-1], ensure_ascii=False))
    con.close()
