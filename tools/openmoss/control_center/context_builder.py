...
def build_stage_context(task_id: str, stage_name: str, contract: Dict[str, object], state: Dict[str, object]) -> Dict[str, object]:
    ...
    skill_action_plane = mission.get("skill_action_plane", {}) or control_center.get("skill_action_plane", {})
    ...
    merged_allowed_tools = sorted(
        dict.fromkeys(
            [str(item) for item in contract.get("allowed_tools", []) if str(item).strip()]
            + crawler_tools
            + [str(item) for item in acquisition_hand.get("recommended_tools", []) if str(item).strip()]
        )
    )
